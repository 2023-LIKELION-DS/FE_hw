...
while True:
    money = input("입금이면 1, 출금이면 2 (종료는 아무거나 누르세요): ")
    if money == "1":
        amount = int(input("금액: "))
        if amount > 0:
            first += amount
            print(f"{amount}원이 입금되었습니다. 현재 잔고는 {first}원 입니다.")
        else:
        ...
        if amount > 0:
            if first >= amount:
                first -= amount
                print(f"{amount}원이 출금되었습니다. 현재 잔고는 {first}원 입니다.")
                if first == 0:
                    print("통장을 파기합니다.")
                    break
            else:
                print(f"현재 잔고 부족입니다. {amount - first}원이 부족합니다.")
        else:
            print("금액을 0보다 크게 적으세요.")
    else:
        break
...
for i in range(1, 7):
    if random.choice([True, False]):
        print(f"{i}번째 테이블에서 {random.choice(menu)}를 주문했습니다.")
...
result = [random.choice(mbti_type) for _ in range(200)]
mbti_count = {}

for mbti_type in mbti_type:
    count = result.count(mbti_type)
    if count > 0:
        mbti_count[mbti_type] = count
