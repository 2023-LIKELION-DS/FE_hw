...
while True:
    ...
    if action not in (1,2) :
        ...
        if action == 1 :
            ...
            print(f"{price}원이 입금되었습니다. 현재 잔고는 {deposit}원 입니다.")
        elif action == 2 :
            if (deposit - price) < 0 :
                print(f"현재 잔고 부족입니다. {price-deposit}원이 부족합니다.")
            elif (deposit - price) == 0 :
                print("통장을 파기합니다.")
                break
            else:
                deposit = deposit - price
                print(f"{price}원이 출금되었습니다. 현재 잔고는 {deposit}원 입니다.")
...
menu = []
while True:
    add_menu = input("추가할 메뉴를 입력하세요.(추가 완료 시 '완료'를 입력하세요.): ")
    if add_menu == "완료" :
        import random
        ...
        for i in range(random_num):
            ...
            print(f"{table}번째 테이블에서 {choice_menu}를 주문했습니다.")
        break
    else: 
