...
while True:
    action = input("입금이면 1, 출금이면 2 (종료는 아무거나 누르세요): ")
    ...
    if action == "1":
        ...
        if amount <= 0:
            ...
            balance += amount
            print(f"{amount}원이 입금되었습니다. 현재 잔고는 {balance}원입니다.")
    elif action == "2":
        amount = int(input("금액: "))
        if amount > balance:
            print(f"현재 잔고 부족입니다. {amount - balance}원이 부족합니다.")
        else:
            balance -= amount
            print(f"{amount}원이 출금되었습니다. 현재 잔고는 {balance}원입니다.")
            if balance == 0:
                print("통장을 파기합니다.")
                break
    else:
...
for table in tables:
    ordered_menu = random.choice(menu)
    print(f"{table}번째 테이블에서 {ordered_menu}를 주문했습니다.")
...
def generate_mbti_list(num):
    mbti_result = []
    for i in range(num):
        mbti_result.append(random.choice(mbti_list))
    return mbti_result

mbti_result = generate_mbti_list(200)
print(mbti_result)
...
mbti_count = count_mbti(mbti_result)
print(mbti_count)
...
def count_mbti_by_key(mbti_list, key):
    mbti_dict = count_mbti(mbti_list)
    key = key.upper()
    if key in mbti_dict:
        return mbti_dict[key]
    else:
        return 0

# 'INFJ' 유형에 해당하는 사람 수 출력
count = count_mbti_by_key(mbti_result, 'INFJ')
print(count)

# 'enfj' 유형에 해당하는 사람 수 출력
count = count_mbti_by_key(mbti_result, 'enfj')
print(count)

# 'ZZZZ' 유형에 해당하는 사람 수 출력
count = count_mbti_by_key(mbti_result, 'ZZZZ')
print(count)
