...
while True:
    select = input("입금이면 1, 출금이면 2 (종료는 아무거나 누르세요): ")
    if select == '1':
        deposit = int(input("금액: "))
        if deposit <= 0:
            print("금액을 0보다 크게 적으세요.")
        else:
            bank = bank + deposit
            print("%d원이 입금되었습니다. 현재 잔고는 %d입니다." %(deposit, bank))
    elif select == '2':
        withdraw = int(input("금액: "))
        if withdraw > bank:
            print("현재 잔고 부족입니다. %d원이 부족합니다." %(withdraw-bank))
        elif withdraw <= 0:
            print("금액을 0보다 크게 적으세요.")
        else:
            bank = bank-withdraw
            if bank == 0:
                ...
                print("%d원이 출금되었습니다. 현재 잔고는 %d원 입니다." %(withdraw, bank))
    else:
        break
...
menus = []
while True:
    y_menu = input("추가할 메뉴를 입력하세요.(추가 완료 시 '완료'를 입력하세요.): ")
    if y_menu != '완료':
        menus.append(y_menu)
        print("메뉴판: ", menus)
    else:
        ...
        for i in range(0, table):
            print("%d번째 테이블에서 %s를 주문했습니다." %(i+1, random.choice(menus)))
        break
...
print("[문제 5-1]")
people = []
def makembti(mbti1, mbti2, mbti3, mbti4, people):
    for i in range(0, 200):
        mbti = random.choice(mbti1) + random.choice(mbti2) + random.choice(mbti3) + random.choice(mbti4)
        people.append(mbti)
makembti("EI", "SN", "FT", "JP", people)
...
print("[문제 5-3]")
def mbtinum(count):
    MBTI = input("몇 명이 속해있는지 궁금한 MBTI입력: ")
    MBTI = MBTI.upper()
    print(count[MBTI])
