...
while(answer==1 or answer==2):
    answer = int(input("입금이면 1, 출금이면 2 (종료는 아무거나 누르세요):"))
    ...
    if (answer == 1):
        ...
        print(f"{amount}원이 입금되었습니다. 현재 잔고는 {account}원 입니다.")
    elif(answer == 2):
        amount = int(input("금액: "))
        if(amount<0):
            print("금액을 0보다 크게 적으세요.")
            break
        if(amount>account):
            print(f"현재 잔고 부족입니다. {amount-account}원이 부족합니다.")
            break
        if(amount==account):
            print("통장을 파기합니다.")
            account = 0
            break
        account = account - amount
        print(f"{amount}원이 출금되었습니다. 현재 잔고는 {account}원 입니다.")
...
menu = []
while(True):
    food = str(input("추가할 메뉴를 입력하세요.(추가 완료 시 '완료'를 입력하세요.): "))
    if(food == "완료"):
        break
    menu.append(food)
...
table_count=random.randrange(0,7)
k=1

for i in range(0,table_count):
    food = menu[random.randrange(0,len(menu))]
    print(f"{i+1}번째 테이블에서 {food}을(를) 주문했습니다.")
...
print("[문제 5-2]")
mbti_count = {'ISTJ': 0, 'ISFJ': 0, 'INFJ': 0, 'INTJ':0, 'ISTP':0, 'ISFP':0, 'INFP':0, 'INTP':0, 'ESTP':0, 'ESFP':0, 'ENFP':0, 'ENTP':0, 'ESTJ':0 ,'ESFJ':0, 'ENTJ':0}
for i in range(0,200):
    if (result[i] in mbti_count.keys()):
        mbti_count[result[i]]+=1
...
mbti_search = input("mbti 유형을 입력하세요: ")

if (mbti_search.upper() in mbti_count.keys()):
    print(f"총 {mbti_count[mbti_search.upper()]}명 입니다.")
else:
    print("잘못 입력하셨습니다.")